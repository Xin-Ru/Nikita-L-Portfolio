function startGame() {
    ...
    //利用 Array.from 和 Array.map 將將 52 張牌加入
    ...
    maingameArea.forEach(function (item, index) {
        if (index <= 3) {
            cardLineGroup[0].push(item);
        } else {
            cardLineGroup[1].push(item)
        };
    });

    function putCard() {
        cardLineGroup.forEach(function (section, sectionNum) {
            let cardLineContainer = document.createElement('div');
            cardLineContainer.className = 'cardLineBox'

            section.forEach(function (item, index) {
                let cardGroup = document.createElement('div');
                cardGroup.className = 'cardLine';
                cardGroup.group = index;
                cardGroup.section = sectionNum;
                item.forEach(function (el, num) {
                    let oneCard = document.createElement('div');
                    oneCard.className = 'cardBox';
                    if (!isRefresh) {
                        oneCard.style.top = '-1000px';
                        oneCard.style.left = '-2000px';
                        setTimeout(function () {
                            oneCard.style.top = num * 43 + 'px';
                    ...
                    cardImg.src = `pokerImgs/${judgeColor(el)}-${el % 13}.png`;
                ...
                cardLineContainer.appendChild(cardGroup);
            })

            gamingArea.appendChild(cardLineContainer);
        });
    ...
    function storefinishCard() {
        finishArea.forEach(function (item, index) {
            ...
            completedDeck.className = 'singleCardBox'
            completedDeck.status = 'finished'
            completedDeck.innerHTML = `<img class="singleCardIcon" src="img/${sortColor(index)}-icon.png">`
            ...
            item.forEach(function (el, num) {
                ...
                cardImg.color = judgeColor(el);
                cardImg.src = `pokerImgs/card-${judgeColor(el)}-${el % 13}.png`;
    ...
    function storetempCard() {
        temporaryArea.forEach(function (item, index) {
            ...
            temporaryDeck.className = 'singleCardBox';
    ...
    /*回朔功能製作*/
    // let goBack = document.getElementById('returnBtn')
    ...
    //         cardLineGroup[backfirstStep.from.cardSection][backfirstStep.from.cardGroup].push(backfirstStep.from.cardNum)
    ...
    //         cardLineGroup[backfirstStep.to.cardSection][backfirstStep.to.cardGroup].pop();
    //         cardLineGroup[backfirstStep.from.cardSection][backfirstStep.from.cardGroup].push(backfirstStep.from.cardNum)
    //     }
    //     refreshWindow()
    // }
    ...
    //     }
    //     if (isTemporary) {
    //         if (temporaryArea[ondropGroup].length == 1) {
    //             return
    //         };
    //         backStep.push({
    //             from: {
    ...
    //             active: 'putinTemp'
    //         });
    //         cardLineGroup[ondragSection][ondragGroup].pop();
    ...
    //                 });
    //                 temporaryArea[ondragGroup].pop();
    //                 cardLineGroup[ondropSection][ondropGroup].push(ondragCard)
    //                 clear()
    //             } else if (cardLineGroup[ondragSection][ondragGroup].length > 0) {
    //                 backStep.push({
    //                     from: {
    //                         cardNum: ondragCard,
    //                         cardGroup: ondragGroup,
    ...
    //                 clear()
    //             }
    //         } else if (cardLineGroup[ondropSection][ondropGroup].length == 0) {
    //             if (temporaryArea[ondragGroup].indexOf(ondragCard) > -1) {
    //                 backStep.push({
    //                     from: {
    //                         cardNum: ondragCard,
